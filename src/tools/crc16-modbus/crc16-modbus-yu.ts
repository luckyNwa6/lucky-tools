...
function crcYu(ptr:number[], len:number) {
	...
	// console.log("校验后的值", crc)
	// 十六进制校验码
	let hexCRC = uint16ToHexString(crc);
	// 大小端互换
	let crcH = [];
	crcH[0] = parseInt(hexCRC[2] + hexCRC[3], 16);
	crcH[1] = parseInt(hexCRC[0] + hexCRC[1], 16);
	// console.log("最终校验结果", Ab2Hex(crcH));
	let crcData:string[] = Ab2Hex(crcH)
	return crcData;
}
 ...
 * 十六进制转码
 */
function uint16ToHexString(number:number) {
	let ret = [];
	let string = '0';
